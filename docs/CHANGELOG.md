...
## v2.8.0
### New features
- Added configurable repetitions for non-RFC2544 tests. Each test defaults to one run, preserves every repetition in the result history, and waits 3 seconds between runs.
- Added generation support for DetNet control worlds with MPLS data planes (8, 16, 28 bit sequence numbers). The sequence number generated by P4TG is moved to this field.
...
- Added controller-orchestrated RFC2544 testing:
